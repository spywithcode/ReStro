...
export default function AiSuggestion() {
    ...
    const [formData, setFormData] = useState<MenuSuggestionsInput>({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSuggestion("");
        try {
            const result = await getMenuSuggestions(formData);
            setSuggestion(result.suggestions);
        } catch (error) {
            console.error("Error getting AI suggestions:", error);
            toast({
                title: "Error",
                description: "Failed to get menu suggestions. Please try again.",
                variant: "destructive",
            });
        } finally {
            setLoading(false);
